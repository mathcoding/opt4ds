...
def SolveTSP_SEC(G, TIME_LIMIT=120):
    ...
    L = lambda a: a if a[0] < a[1] else (a[1], a[0])
    Delta = lambda i: map(L, G.edges(i))
    # Node degree
    for i in G.nodes():
        model.addConstr(quicksum(x[e] for e in Delta(i)) == 2)

    # Manual addition
    model.addConstr(x[8,11] + x[3,8] + x[3,11] + x[8,18] + x[11, 14] + x[3, 13] <= 3 + 1)
