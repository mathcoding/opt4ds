...
def PlotTour(Ps, Ls, values):
    ...
    lc = mc.LineCollection(lines, linewidths=[1.5 if x > 0.99 else 1 for x in values],
                           colors=['blue' if x > 0.99 else 'red' for x in values], alpha=0.5)
...
# Solve the LP relaxation of the ATSP
def SolveTSP_SEC(G, TIME_LIMIT=1200):
    model = Model()
    ...
    # TODO: add a max number of iterations to avoid looping forever
    while True:
        # Solve model the LP relaxation
        model.optimize()
        ...
        # Support solution
        xbar = {(i,j): x[i,j].x for i,j in x if x[i,j].x > 0.001}
    
        # Easy check: compute connected components of the underlying undirected graph
        H = nx.Graph()
        H.add_edges_from([(i,j) for i,j in xbar])
        Subtours = sorted([list(c) for c in nx.connected_components(H)], key=len)
    
        if len(Subtours) > 1:
            # Add cuts and iterate
            for cycle in Subtours:
                print('disconnected component:', cycle)
                model.addConstr(quicksum(x[i,j] for i in cycle for j in range(n) if j not in cycle) >= 1)
        else:
            # This operation costs more ...
            S = FindMinCut(xbar)
            if S is None:
                return model.objVal, [(i,j) for i,j in x if x[i,j].X > 0.01], [x[i,j].x for i,j in x if x[i,j].x > 0.01]
...
if __name__ == "__main__":
    ...
    z_lp, tour, values = SolveTSP_SEC(G) 
